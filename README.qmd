...
- Communication: Karl Berator - Research Assistant, Institute of -omics, Adelaide

- Communication type / updates: Email
...
#### Emailed Karl the following questions on 03/03/2023:

- What the sheet names represent

- The concentration units used

- Whether the -99 entry is an NA value or an error point that will need to be looked at. 


#### Reply received on 03/03/2023:

- The sheet names are the gene line we used. 

- The concentration units are mg/ml. 

- Yes, we use -99 to indicate missing data. 
...
- Removed the -99 data point (used to indicate missing data)
...
- Made tables to separate this further by cell line (wild-type and cell-type 101). In both cases, the mean gene expression was greater for Activating Factor 42 than placebo, and increased as concentration increased.
...
- Format the gene line names using `substr()` 
- Created a custom colour palette - `colours <- c("#78A8D1", "#D6C099")`
- Added Times New Roman (can be seen in `figures/times.ttf`)
...
Report and analysis in file `2023_05_16-Gene_expression_IMRaD.qmd`.
...
- Found the coefficients using `fixef()` and `ranef()`, and included these as a table in the report.

- A table of the metrics AIC, r-squared and RMSE was also included for the final model, $m3$.
